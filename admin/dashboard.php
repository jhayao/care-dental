...
function loadDashboard() {
    ...
    .then(data => {
        ...
        // Since we explicitly showed Regular, Senior, PWD, we might not need this loop if those are the only ones.
        // But good to keep for any custom future categories.
        const categoryCards = document.getElementById('categoryCards');
        categoryCards.innerHTML = '';
        const categoryColors = ['#f59e0b', '#10b981', '#3b82f6', '#ef4444'];
        let catIndex = 0;
        for (const category in data.categories) {
            if (category === 'None' || category === 'Senior' || category === 'PWD') continue; // Skip already shown
            const card = document.createElement('div');
    ...
    // --- Fetch Income Data ---
    fetch(`get_income_data.php?start_date=${startDate}&end_date=${endDate}`)
    .then(res => res.json())
    .then(data => {
