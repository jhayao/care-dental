...
function handleCreatePayment(e) {
    ...
    // Note admin needs to post to staff path because we only made one file?
    // Or I can copy the file to admin.
    // For now, let's point to staff file, assuming layout allows relative or absolute.
    // We are in /admin, file is in /staff. Path: ../staff/create_payment_request.php
    
    fetch('../staff/create_payment_request.php', {
        method: 'POST',
        body: formData
    })
    .then(res => res.json())
